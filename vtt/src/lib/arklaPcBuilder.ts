...
interface RawArklaItem {
  name?: string;
  quantity?: number;
  desc?: string;
}

interface RawArklaFeature {
  name?: string;
  desc?: string;
  source?: string;
}

interface RawArklaChar {
  name?: string;
  label?: string;
  playerName?: string;
  species?: string;
  ...
  hitPoints?: { current: number; max: number; temporary: number } | number;
  maxHp?: number;
  hitDice?: { type?: string; max?: number };
  proficiencyBonus?: number;
  conditions?: string[];
  deathSaves?: { successes: number; failures: number };
  temporaryHitPoints?: number;
  languages?: string[];
  features?: RawArklaFeature[];
  inventory?: RawArklaItem[];
  currency?: {
    leptons?: number; copper?: number;
    assarions?: number; silver?: number;
    bronzeDrakes?: number; electrum?: number;
    silverDrakes?: number; gold?: number;
    goldCrowns?: number; platinum?: number;
  };
  appearance?: string;
  backstory?: string;
  allies?: string;
...
export function buildPcsFromArkla(characters: Record<string, RawArklaChar>): PlayerCharacter[] {
  return Object.entries(characters).map(([id, raw]) => {
    const stats = raw.stats ?? {};
    const speedVal = raw.speed ?? 30;
    const hp = raw.hitPoints ?? { current: raw.maxHp ?? 10, max: raw.maxHp ?? 10, temporary: 0 };
    const inventory = (raw.inventory ?? []).map((item) => ({
    ...
    return {
      ...
      classes: [{ name: className, subClass: raw.subClass ?? "", level: classLevel, hitDice: hitDieType, classFeatures: [] as never[] }],
      class: className,

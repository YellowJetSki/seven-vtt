/**
 * STᚱ VTT — Mobile Bottom Nav (Premium — All Routes)
 *
 * Mobile bottom navigation with ALL routes available.
 * This ensures mobile users can navigate to ANY page,
 * not just the 5 most common ones.
 *
 * Design:
 * - Horizontally scrollable if items overflow (max 5 visible)
 * - Gold-accented active states with dot indicator
 * - Glass background with top gold gradient border
...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-obsidian/95 backdrop-blur-lg border-t border-gold/10 sm:hidden safe-area-bottom">
      ...
      <div
        ref={scrollRef}
        className={`flex items-center overflow-x-auto scrollbar-gold ${canScroll ? "px-1" : "justify-around px-2"} py-1`}
      >
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-2.5 py-2 rounded-xl transition-all duration-200 active:scale-95 relative shrink-0 ${
                isActive
                  ? "text-gold-400"
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={`text-lg ${isActive ? "drop-shadow-[0_0_6px_rgba(234,179,8,0.15)]" : ""}`}>
                  {item.icon}
                </span>

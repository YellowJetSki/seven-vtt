 ...
 *
 * Added Sprint 22:
 * - Visible grab handle (☰) with drag affordance
 * - `isDragging` prop: reduced opacity + left border glow
 * - `isDropTarget` prop: subtle background highlight
 * - `onDragEnd` prop: cleanup on drag abort
 * - Drag handle targets the grab handle area specifically for precise reordering
 *
 * Mechanical upgrades (existing):
 * - AC badge always visible
 * - Color-coded HP bar with fraction
 * - Temporary HP indicator
 * - Status effects as color-coded badges
 * - Dead/Unconscious visual state (line-through + opacity)
 * - Quick HP input with ±1/±5 presets
 */
...
export default function InitiativeCombatantRow(props: InitiativeCombatantRowProps) {
  ...
  // ── Handle drag start from the grab handle specifically ──
  const handleDragStart = useCallback(
    (e: DragEvent) => {
      // Set drag data and ghost
      e.dataTransfer.effectAllowed = "move";
      e.dataTransfer.setData("text/plain", String(index));

      // Slight delay to let the browser snapshot the drag image
  ...
  return (
    <div
      ...
      className={`group relative rounded-lg transition-all duration-200 select-none ${
        isCurrentTurn
          ? "bg-gold-500/8 ring-1 ring-gold/25 shadow-[0_0_12px_rgba(234,179,8,0.04)]"
          : isDropTarget
          ? "bg-gold-500/6 ring-1 ring-gold/15"
        ...
        isDragging ? "opacity-40 border border-dashed border-gold-500/20" : ""
      } p-2 pl-0`}
    >
      {/* ── Grab handle area (drag source) ── */}
      <div
        ref={grabHandleRef}
        draggable
        onDragStart={handleDragStart}
        onDragEnd={handleDragEnd}
        className="absolute left-0 top-0 bottom-0 w-5 flex items-center justify-center cursor-grab active:cursor-grabbing hover:bg-gold-500/5 rounded-l-lg transition-colors duration-150 group/drag"
      >
        <div className="flex flex-col items-center gap-[1.5px] opacity-40 group-hover/drag:opacity-80 group-active/drag:opacity-100 transition-opacity duration-150">
          <div className="w-1.5 h-[1.5px] rounded-full bg-surface-500" />
          <div className="w-1.5 h-[1.5px] rounded-full bg-surface-500" />
          <div className="w-1.5 h-[1.5px] rounded-full bg-surface-500" />
        </div>
      </div>

      {/* ── Pl-5 to account for the grab handle ── */}
      <div className="pl-5">
        {/* Current turn gold pulse dot */}
        {isCurrentTurn && (
          <div className="absolute left-[14px] top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-gold-400 rounded-full animate-pulse-soft shadow-[0_0_6px_rgba(234,179,8,0.4)]" />
        )}
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Mini HP bar */}
          <div className="w-9 h-1 bg-[#07080d] rounded-full overflow-hidden shrink-0">
            <div

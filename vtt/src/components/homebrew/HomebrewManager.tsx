/**
 * STᚱ VTT — Homebrew Manager (Premium v3.2 — Enemies Integration)
 *
 * Full CRUD panel for homebrew items, spells, feats, and enemies with
 * bulk operations, export/import, SRD reference display, visibility
 * control, and duplicate. Integrated with campaignStore enemies.
 */
...
export default function HomebrewManager() {
  ...
  const [enemyFormOpen, setEnemyFormOpen] = useState(false);
  const [editingEnemy, setEditingEnemy] = useState<EnemyDoc | undefined>();

  const {
    formMode, formType, itemForm, spellForm, featForm,
    featBenefitsInput,
    setItemForm, setSpellForm, setFeatForm, setFeatBenefitsInput,
    openForm, closeForm, submitItem, submitSpell, submitFeat,
  } = useHomebrewForms();
  ...
  const allEnemies = useMemo(() => [...enemies], [enemies]);

  // ── Filtered lists ──
  const filteredItems = useMemo(() => filterList(allItems, search), [allItems, search]);
  const filteredSpells = useMemo(() => filterList(allSpells, search), [allSpells, search]);
  const filteredFeats = useMemo(() => filterList(allFeats, search), [allFeats, search]);
  const filteredEnemies = useMemo(() => filterEnemyList(allEnemies, search), [allEnemies, search]);

  const tabPlaceholder = activeTab === "items" ? "Search items..."
    : activeTab === "spells" ? "Search spells..."
    : activeTab === "feats" ? "Search feats..."
    : "Search monsters...";

  const tabCount = activeTab === "items" ? filteredItems.length
    : activeTab === "spells" ? filteredSpells.length
    : activeTab === "feats" ? filteredFeats.length
  ...
  const handleDuplicateItem = useCallback(
    (item: HomebrewItem) => {
      ...
      const copy: HomebrewItem = {
        ...item, id: `dup_${now}_${Math.random().toString(36).slice(2, 6)}`,
        name: `${item.name} (Copy)`, isHomebrew: true, source: "homebrew",
  ...
  const handleDuplicateSpell = useCallback(
    (spell: HomebrewSpell) => {
      ...
      const copy: HomebrewSpell = {
        ...spell, id: `dup_${now}_${Math.random().toString(36).slice(2, 6)}`,
        name: `${spell.name} (Copy)`, isHomebrew: true, source: "homebrew",
        createdAt: now, updatedAt: now,
      };
      store.addSpell(copy);
  ...
  const handleDuplicateFeat = useCallback(
    (feat: HomebrewFeat) => {
      ...
      const copy: HomebrewFeat = {
        ...feat, id: `dup_${now}_${Math.random().toString(36).slice(2, 6)}`,
        name: `${feat.name} (Copy)`, isHomebrew: true, source: "homebrew",
        createdAt: now, updatedAt: now,
      };
      store.addFeat(copy);
  ...
  const handleDuplicateEnemy = useCallback(
    (enemy: EnemyDoc) => {
      const now = Date.now();
      const copy: EnemyDoc = {
        ...enemy, id: `dup_${now}_${Math.random().toString(36).slice(2, 6)}`,
        name: `${enemy.name} (Copy)`, isHomebrew: true,
        createdAt: now, updatedAt: now,
      };
      setEnemies([...enemies, copy]);
      showToast({ message: `"${enemy.name}" duplicated`, type: "success" });
    }, [enemies, setEnemies]
  );

  // ── Enemy form management ──
  const handleOpenEnemyForm = useCallback((enemy?: EnemyDoc) => {
    setEditingEnemy(enemy);
    setEnemyFormOpen(true);
  }, []);
  const handleCreateEnemy = useCallback(
    (enemy: EnemyDoc) => {
      setEnemies([...enemies, enemy]);
      setEnemyFormOpen(false);
      setEditingEnemy(undefined);
      showToast({ message: `"${enemy.name}" created`, type: "success" });
    }, [enemies, setEnemies]
  );
  ...
  const toggleItemVisibility = useCallback(
    (id: string, visible: boolean) => {
      ...
      if (item) { store.removeItem(id); store.addItem({ ...item, visibleToPlayers: visible, updatedAt: Date.now() }); }
    }, [items, store]
  );
  const toggleSpellVisibility = useCallback(
    (id: string, visible: boolean) => {
      const spell = spells.find((s) => s.id === id);
      if (spell) { store.removeSpell(id); store.addSpell({ ...spell, visibleToPlayers: visible, updatedAt: Date.now() }); }
    }, [spells, store]
  );
  const toggleFeatVisibility = useCallback(
    (id: string, visible: boolean) => {
      const feat = feats.find((f) => f.id === id);
      if (feat) { store.removeFeat(id); store.addFeat({ ...feat, visibleToPlayers: visible, updatedAt: Date.now() }); }
    }, [feats, store]
  );
  const toggleEnemyVisibility = useCallback(
    (id: string, _visible: boolean) => {
      // Touch timestamp for enemies (no visibility API on EnemyDoc)
      setEnemies(enemies.map((e) => e.id === id ? { ...e, updatedAt: Date.now() } : e));
    }, [enemies, setEnemies]
  );
  ...
  const handleDeleteEnemy = useCallback(
    (id: string) => {
      const name = enemies.find((e) => e.id === id)?.name ?? "Monster";
  ...
  const handleBulkDelete = useCallback(() => {
    ...
    selectedIds.forEach((id) => {
      ...
      if (item) { store.removeItem(id); deletedNames.push(item.name); }
      else if (spell) { store.removeSpell(id); deletedNames.push(spell.name); }
      else if (feat) { store.removeFeat(id); deletedNames.push(feat.name); }
      else if (enemy) { setEnemies(enemies.filter((e) => e.id !== id)); deletedNames.push(enemy.name); }
    });
  ...
  }, [selectedIds, items, spells, feats, enemies, store, setEnemies]);
  ...
  return (
    <div style={{ animation: "slide-in-up 0.35s ease-out both" }}>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {activeTab !== "enemies" && (
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={showSRD} onChange={(e) => setShowSRD(e.target.checked)}
      ...
      <HomebrewSearchBar
        ...
        onAdd={() => activeTab === "enemies" ? handleOpenEnemyForm() : openForm("add", activeTab)}
      ...
      {/* Count */}
      <div className="text-[10px] text-surface-500 mb-3 tabular-nums">{tabCount} {activeTab}</div>
      ...
      <HomebrewTabPanel
        ...
        onEditEnemy={(e) => handleOpenEnemyForm(e)}
        onDeleteItem={(id) => {
          const name = items.find((i) => i.id === id)?.name ?? "Item";
          store.removeItem(id); showToast({ message: `"${name}" deleted`, type: "warning" });
        }}
        onDeleteSpell={(id) => {
          const name = spells.find((s) => s.id === id)?.name ?? "Spell";
          store.removeSpell(id); showToast({ message: `"${name}" deleted`, type: "warning" });
        }}
        onDeleteFeat={(id) => {
          const name = feats.find((f) => f.id === id)?.name ?? "Feat";
          store.removeFeat(id); showToast({ message: `"${name}" deleted`, type: "warning" });
        }}
        onDeleteEnemy={handleDeleteEnemy}
        onDuplicateItem={handleDuplicateItem}
        onDuplicateSpell={handleDuplicateSpell}
        onDuplicateFeat={handleDuplicateFeat}
        onDuplicateEnemy={handleDuplicateEnemy}
        ...
        onToggleEnemyVisibility={toggleEnemyVisibility}
        isBulkMode={isBulkMode}
      ...
      {/* Enemy creation modal — direct EnemyCreator (has its own glass modal) */}
      <EnemyCreator
        isOpen={enemyFormOpen}
        onClose={() => { setEnemyFormOpen(false); setEditingEnemy(undefined); }}
        onCreated={handleCreateEnemy}
        onSaved={handleCreateEnemy}
        existingEnemy={editingEnemy}
      />
    </div>
  );
}

/** Search filter for items/spells/feats */
...
/** Search filter for enemies by name, type, traits */
function filterEnemyList(list: EnemyDoc[], search: string): EnemyDoc[] {
  const q = search.toLowerCase().trim();
  if (!q) return list;
  return list.filter((entry) => {
    if (entry.name.toLowerCase().includes(q)) return true;
    if (entry.type.toLowerCase().includes(q)) return true;
    if (entry.senses?.toLowerCase().includes(q)) return true;
    if (entry.languages?.toLowerCase().includes(q)) return true;
    if (entry.traits?.toLowerCase().includes(q)) return true;
    if (entry.actions?.toLowerCase().includes(q)) return true;
    return false;
  });
}

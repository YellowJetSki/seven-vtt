/**
 * STᚱ VTT — Campaign Stats Dashboard
 *
    ...
    <SettingsSection icon="📊" title="Campaign Statistics" description="Live counts from your campaign data">
      ...
      <div className="grid grid-cols-5 gap-2">
        {STAT_CARDS.map((stat) => (
          <div
            key={stat.key}
            className="bg-[#07080d] border border-white/[0.04] rounded-xl p-2.5 text-center hover:border-gold-500/10 transition-colors"
          >
            <div className="text-lg">{stat.icon}</div>
            <div className={`text-base font-bold mt-0.5 ${stat.color}`}>
              {liveStats[stat.key]}
            </div>
      ...
      {/* Session counter */}
      <div className="flex items-center justify-between bg-[#07080d] border border-white/[0.04] rounded-xl p-3">
        <div className="flex items-center gap-2">
          <span className="text-lg">🎲</span>
          <div>
            <div className="text-sm font-bold text-gold-300">{sessionCount}</div>
            <div className="text-[9px] text-surface-600">Total Sessions</div>
          </div>
        </div>
        <button
          onClick={onIncrementSession}
          className="px-3 py-1.5 rounded-lg text-[10px] font-bold bg-gold-500/10 border border-gold/15 text-gold-400 hover:bg-gold-500/15 active:scale-95 transition-all duration-150"
        >
          + New Session
        </button>
      </div>
      ...
      {meta && (
        <div className="text-[9px] text-surface-600 text-center pt-1">
          Campaign created {new Date(meta.createdAt).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
          {meta.updatedAt !== meta.createdAt && ` · Last updated ${new Date(meta.updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`}
        </div>
      )}
    </SettingsSection>
  );

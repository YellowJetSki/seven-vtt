...
function convertLegacyChar(lc: LegacyChar): PlayerCharacter {
  return {
    ...
    proficiencies: [],
    languages: [],
    features: lc.features.map((f: string) => ({ name: f, description: f, source: "Class" })),
    ...
    savingThrows: {
      strength: { proficient: false, bonus: Math.floor((lc.abilityScores.strength - 10) / 2) },
      dexterity: { proficient: true, bonus: Math.floor((lc.abilityScores.dexterity - 10) / 2) + lc.proficiencyBonus },
      constitution: { proficient: false, bonus: Math.floor((lc.abilityScores.constitution - 10) / 2) },
      intelligence: { proficient: false, bonus: Math.floor((lc.abilityScores.intelligence - 10) / 2) },
      wisdom: { proficient: true, bonus: Math.floor((lc.abilityScores.wisdom - 10) / 2) + lc.proficiencyBonus },
      charisma: { proficient: false, bonus: Math.floor((lc.abilityScores.charisma - 10) / 2) },
    },
    skills: {
      acrobatics: "proficient", animalHandling: "none", arcana: "none",
      athletics: "none", deception: "none", history: "none",
      insight: "proficient", intimidation: "none", investigation: "none",
...
const pollCaveMap: BattleMap = {
  ...
  imageUrl: "/images/battlemaps/screwbeard_cave_enc.png",
  imageFit: "cover",
  gridWidth: 47,
  gridHeight: 26,
  gridSize: 40,

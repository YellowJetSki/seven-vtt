...
export default function BattleMapsPage() {
  ...
  const handleCellClick = useCallback((gridX: number, gridY: number) => {
    ...
    if (placementMode === "light") {
      ...
      if (last) { setLights((p) => { const u = [...p]; u[u.length - 1] = { ...last, x: gridX * gs + gs / 2, y: gridY * gs + gs / 2 }; return u; }); }
    ...
    if (placementMode === "wall" || placementMode === "door") {
      ...
      handleAddWall({ id, x1: wallStart.x * gs, y1: wallStart.y * gs, x2: gridX * gs, y2: gridY * gs, blocksVision: true, blocksMovement: placementMode === "wall", blocksLight: placementMode !== "door", isDoor: placementMode === "door", isWindow: false, doorState: placementMode === "door" ? "closed" : undefined });
      setWallStart(null);
      setPlacementMode("none");
    }
  }, [placementMode, wallStart, selectedMap, lights]);

  if (battleMaps.length === 0) return (
    <AppShell><EmptyState icon="🗺" title="No Battle Maps Yet" description="Create your first battle map to start using the Canvas-based rendering engine with dynamic lighting, fog of war, and wall-based line-of-sight." /></AppShell>
  );

  if (!selectedMap) return (
    <AppShell>
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-white">Battle Maps</h1>
  ...
  return (
    <AppShell>
      <div className="flex flex-col h-[calc(100vh-8rem)]">
        <MapToolbar placementMode={placementMode} mapName={selectedMap.name} onBack={() => setSelectedMapId(null)} onSetPlacementMode={(m) => { setPlacementMode(m); setWallStart(null); }} />
        <div className="flex gap-4 flex-1 min-h-0">
          <div className="flex-1 min-w-0">
            <CanvasMapView mapData={selectedMap} tokens={tokens} lights={lights} walls={walls} dmView={true} onCellClick={handleCellClick} onTokenClick={(t) => console.log("Token:", t)} />
          </div>

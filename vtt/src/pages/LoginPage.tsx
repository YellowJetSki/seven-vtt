...
export function LoginPage() {
  ...
  const handleDmLogin = async (e: FormEvent) => {
    ...
    // 1. Authenticate against app-level env credentials
    ...
    // 2. Authenticate against Firebase Auth for secure Firestore access
    setFirebaseAuthLoading(true);
    const firebaseOk = await loginFirebaseDm();
    setFirebaseAuthLoading(false);

    if (!firebaseOk) {
      // App-level auth succeeded but Firebase Auth failed.
      // Still allow login but mark Firebase as disconnected.
  ...
  const handlePlayerLogin = async (e: FormEvent) => {
    ...
    // 1. Authenticate against campaign's player character list
    ...
    // 2. Sign in anonymously to Firebase Auth for read-only Firestore access
    ...
    if (firebaseOk) {
      ...
      setFirebaseConnected(false);
      showToast({
        message: `Welcome, ${playerName.trim()}! (Offline mode — cloud sync unavailable)`,
        type: "info",
      });
    }
        ...
        <div className="rounded-2xl border border-surface-700/50 bg-surface-900/80 p-8 shadow-2xl backdrop-blur-xl">
          ...
          {mode === "dm" && (
            <form onSubmit={handleDmLogin} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={dmLoading || !dmUsername || !dmPassword}
            ...
            <form onSubmit={handlePlayerLogin} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Edmund or Strider"
                  required
